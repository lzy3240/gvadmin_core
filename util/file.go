...
// FileNameToMD5 split filename to md5
func FileNameToMD5(fileName string) (string, error) {
	tmp := strings.Split(fileName, ".")
	h := md5.New()

	if tmp[0] == "" || tmp[len(tmp)-1] == "" {
		return "", errors.New("file name wrong")
	}
	...
	_, _ = io.WriteString(h, strings.Join(tmp[:len(tmp)-1], "."))
	sum := h.Sum(nil)
	return hex.EncodeToString(sum[:]) + "." + tmp[len(tmp)-1], nil
}
